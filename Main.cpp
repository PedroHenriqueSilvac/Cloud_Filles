...
namespace Control_Var//套用到菜单的调试变量 (例如功能开关)
{
	...
	void LoadCloudPreset(string FileName = "", string NormalURL = "https://github.com/PedroHenriqueSilvac/Cloud_Filles/") noexcept//加载特定Github云预设
	{
	...
	while (LanguageID == 0x804 || LanguageID == 0x404 || LanguageID == 0xC04)//中文版菜单 (字符串一定要加上UTT不然会乱码)
	{
		...
		if (!GUI_VAR.Window_Move() && Menu_Open)//不在移动窗口时绘制GUI
		{
			...
			else if (UI_Panel == 3)//Setting
			{
				...
				GUI_VAR.GUI_Text(Block_About, 4, "作者: https://github.com/PedroHenriqueSilvac", { 100,100,100 });
				...
				if (OpenQQGroupChat)System::Open_Website("https://pedrohbss.rf.gd");//打开QQ加入群聊链接
	...
	while (true)//默认英文版菜单
	{
		...
		if (!GUI_VAR.Window_Move() && Menu_Open)//不在移动窗口时绘制GUI
		{
			...
			else if (UI_Panel == 4)//Setting
			{
				...
				if (OpenGithubURL)System::Open_Website("www.nety.com");//打开作者Github主题页面
